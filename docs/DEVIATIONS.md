...
4. The indexer follows the Core best chain immediately. The configurable confirmation count is a settled-display threshold, not a delay in authoritative state.
...
7. Orphan Circle facts and members remain inspectable, while derived orphan lineages and shards are removed by undo. Authoritative API queries exclude them unless orphan status is requested.
